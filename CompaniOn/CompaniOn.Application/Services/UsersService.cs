...
using CompaniOn.Shared.Services;
...
namespace CompaniOn.Application
{
    public class UsersService : BaseService<User, UserDto, UserUpsertDto, UsersSearchObject, IUsersRepository>, IUsersService
    {
        ...
        public async Task<List<UserDto>?> GetBasicUsersOfFamilyMemeber(int userId, CancellationToken cancellationToken = default)
        {
            var user = await CurrentRepository.GetBasicUsersOfFamilyMemeber(userId, cancellationToken);
        ...
        public async Task<UserDto> ChangeVisibilityAsync(int userId, bool isVisible, CancellationToken cancellationToken = default)
        {
            var user = await CurrentRepository.ChangeVisibilityAsync(userId, isVisible, cancellationToken);
            if (user == null)
                throw new UserNotFoundException();
